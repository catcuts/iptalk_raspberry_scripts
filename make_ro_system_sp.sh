...
echo -e "[ info ] 检查硬盘 ..."
    ...
    if [ -z $selected_hd ]; then
        echo -e "\033[31m\033[01m\033[05m[ erro ]\033[0m 没有可用的硬盘 . 中止 ."
        exit 1
    fi
...
step1(){
    ...
    echo -ne "[ info ] 导出 iptalk 数据库 ..."
        ...
        if [ "$iptalksqlsize" == "0" ]; then
            echo -e "\n\033[31m\033[01m\033[05m[ erro ]\033[0m 导出 iptalk 数据库 失败 . 中止 . 检查后再试 ."
            exit 1
        fi
...
check(){
    ...
    if [ -d $1 ]; then
        ...
        if [ "$checked" == "yes" ]; then
            ...
            echo -e "\033[31m\033[01m\033[05m[ erro ]\033[0m 用户模棱两可($checked 而非 yes/no). 中止 . 下定决心后再试 ."
        fi
    else
...
step3(){
    ...
    if [ -n "$mounted" ]; then
        ...
        mounted=`df -h | grep $selected_hd | awk '{print$6}'`
        if [ -n "$mounted" ]; then
            echo -e "\n\033[31m\033[01m\033[05m[ erro ]\033[0m 卸载硬盘失败. 中止 . 手动卸载后重试 ."
            exit 1
        else
            echo -e "好了 ."
        fi
    ...
    if [ "$HDTYPE" != "ext4" ]; then
        echo -e "\033[31m\033[01m\033[05m[ erro ]\033[0m 硬盘不是 ext4 格式. 中止 . 自行格式化后重试 ."
        exit 1
    fi
    ...
    if [ "$mounted" != "/home/pi/hd" ]; then
        echo -e "\n\033[31m\033[01m\033[05m[ erro ]\033[0m 挂载硬盘失败. 中止 . 检查后重试 ."
        exit 1
    else
        echo -e "好了 ."
    fi
...
step4(){
    ...
    echo -ne "[ info ] 是否正确 ? [y/n] "
        read checked
        if [ "$checked" != "y" ]; then
            echo -e "\033[31m\033[01m\033[05m[ erro ]\033[0m 用户检查到错误, 中止 ."
            exit 1
        fi
}
...
step9(){
    ...
    read successed
    if [ "$successed" != "y" ]; then
        echo -e "\033[31m\033[01m\033[05m[ erro ]\033[0m 用户确认试运行失败, 中止 ."
        exit 1
    fi
}
